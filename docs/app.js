...
function visibleProjects() {
  ...
  return state.data.projects.filter((p) => {
    ...
    if (!q) return true;
    return `${p.id} ${p.title} ${p.stack} ${p.entry}`.toLowerCase().includes(q);
  });
}
...
function renderStats() {
  ...
  $("#updated").textContent = ts
    ? `最近一次 Action 回写 · ${ts} · 目标 GPU ${state.data.gpu_target || "T4"}`
    : "等待第一次 T4 回写…";
}
...
function openDrawer(id) {
  ...
  $("#d-facts").innerHTML = `
    ...
    <dt>GPU</dt><dd>${p.gpu || "NVIDIA Tesla T4"}</dd>
    <dt>更新</dt><dd>${p.updated_at || "—"}</dd>
  `;
  $("#d-log").textContent = p.log || "暂无日志";
}
